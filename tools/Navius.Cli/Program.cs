...
for (int i = 0; i < args.Length; i++)
{
    switch (args[i])
    {
        ...
        case "--registry": registryPath = args[++i]; break;
        default: positional.Add(args[i]); break;
    }
}

registryPath ??= Path.Combine(root, "registry", "registry.json");
...
int Add(string name)
{
    ...
    foreach (var n in resolved)
    {
        ...
        foreach (var f in item.GetProperty("files").EnumerateArray())
        {
            ...
            var src = Path.Combine(root, srcRel.Replace('/', Path.DirectorySeparatorChar));
            var dst = Path.Combine(to, tgtRel.Replace('/', Path.DirectorySeparatorChar));

            if (!File.Exists(src))
            {
                Console.Error.WriteLine($"  ! missing source: {srcRel}");
                continue;
            }
